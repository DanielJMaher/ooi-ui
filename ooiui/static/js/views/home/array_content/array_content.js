...
var ParentView = Backbone.View.extend({
    cameraControls: {originalZoom:1.3,resetPitch:0,resetBearing:0},
...
var ArrayContentSummaryItem = ParentView.extend({
    ...
    _flyBye: function(event) {
        map.flyTo({center:[-90, 5],
            maxBounds: [[-179,-80],[0,80]],
            zoom: flyFlyContext.cameraControls.originalZoom,
            speed: 1.,
            pitch: 0, 
            bearing: 0});
        map._setArrayView();
    ...
    _flyFly: _.debounce(function(event) {
        ...
        if (code === 'CE') {
            ...
            if ( _compareGeoLoc(map.getCenter(), [-90, 5]) ) {
                flyFlyContext.cameraControls.originalZoom = map.getZoom();
                //flyByeContext.cameraControls.originalZoom = map.getZoom();
                map.flyTo({center: loc, zoom: 6});
                flyFlyContext._addPopup(loc, name);
            } else {
                console.log('First', map.getZoom());
                map.flyTo({center:[-90, 5],
